...
export function Modal({ title, children, onClose }: { title: string; children: ReactNode; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/60 p-5" role="presentation" onClick={onClose}>
      <div className="max-h-[min(760px,90vh)] w-[min(980px,100%)] min-w-0 rounded-[28px] border border-slate-400 bg-slate-100 p-2 shadow-[0_28px_80px_rgba(15,23,42,0.28)]" role="dialog" aria-modal="true" aria-labelledby={titleId} onClick={(event) => event.stopPropagation()}>
