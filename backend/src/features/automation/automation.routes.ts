...
automationRouter.post("/workflows", requireRole("admin"), (req, res) => {
  ...
  if (dataSource.type === "webhook" || dataSource.type === "mqtt") return res.status(400).json({ error: "Push sources receive data and cannot be polled by automation workflows" });
