...
|---|---|---|
| `auth` | Partial | Covered: `password.service.ts` (PIN/password validation + bcrypt hashing), `session.service.ts` (create/validate/expiry/idle-timeout/cookie options), `auth.service.ts` (`login`/`changePassword`, password-only path). Deliberately skipped: `totp.service.ts` — dead code, `TOTP_ENABLED = false`. Not started: `auth.repository.ts` setup-pending flow, `setup.service.ts`, `auth.middleware.ts`, `audit.service.ts`, `integritas-validation.service.ts` (mostly commented out already). |
