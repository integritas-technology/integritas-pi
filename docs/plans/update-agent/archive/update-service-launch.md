...
**CI → `integritas-manifests` auth is a GitHub App, not a deploy key.** The org disables write-access SSH deploy keys org-wide (a real, deliberate security policy — deploy keys don't expire and can't be passphrase-protected, so a leaked one grants standing write access with no easy revocation). Using a GitHub App (`integritas-pi-manifest-deploy`, installed only on `integritas-manifests`, `Contents: Read and write`) instead — CI generates a short-lived (1 hour) token per run via `actions/create-github-app-token`, no long-lived secret involved in the push itself.

**Done:**
- Create the dedicated low-privilege QA VPS deploy user (`qa-manifest-deploy`) with SSH access.
- Register initial GH repo secrets (`MANIFEST_SIGNING_KEY`; the SSH-push secrets `QA_VPS_DEPLOY_HOST`/`USER`/`KEY`/`PATH` were also registered but are now superseded by this transport change — since deleted, no longer present).
- Create the private `integritas-manifests` repo, with `integritas-pi/qa/` and `integritas-pi/prod/` subfolders.
- Create GitHub App `integritas-pi-manifest-deploy`, install it on `integritas-manifests` only, register `MANIFEST_APP_ID`/`MANIFEST_APP_PRIVATE_KEY` GH secrets on `integritas-pi`.
- Replace the `release.yml` "Deploy manifest" step with the GitHub-App-token-authenticated push step.

**Not done** (full detail and rationale in [manifest-deploy-pull-model.md](./manifest-deploy-pull-model.md)):
- On the QA VPS: clone `integritas-manifests` to `/srv/update-manifests/repo/` (outside the Next.js app's project directory — not `next build`/PM2-managed, so redeploys can't wipe it) using a read-only credential, separate from CI's write access.
